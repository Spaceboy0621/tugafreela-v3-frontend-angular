...
export class FormBankAccountComponent implements OnInit {
  ...
  @Output() sendActiveNav = new EventEmitter();

  formBankAccount: FormGroup;
  pf: boolean = false;
  pj: boolean = false;
  cpfInvalid: boolean;
  bankList: Array<BankListDto>;
  corrente: string;
  poupanca: string;
  ...
  ngOnInit(): void {
    ...
    if(this.typeForm === 'update') {
      this.formBankAccount = this.fb.group({
        pf_pj: [this.bank_account.pf_pj, Validators.required],
        name_of_holder: [this.bank_account.name_of_holder, Validators.required],
        bank: [this.bank_account.bank, Validators.required],
        nif: [(typeof this.bank_account.owner === 'number' ? '' : this.bank_account.owner.nif), Validators.required],
        owner: [null],
        swift: [this.bank_account.swift, Validators.required],
        iban: [this.bank_account.iban, Validators.required],
        iban_code: [this.bank_account.iban_code, Validators.required]
      });
    }
    else if (this.typeForm === 'add') {
      this.formBankAccount = this.fb.group({
        pf_pj: ['', Validators.required],
        name_of_holder: ['', Validators.required],
  ...
  create() {
    ...
    this.formBankAccount.value.owner = this.user.id;

    this.bankAccountService.create(this.formBankAccount.value).subscribe(
      success => {
        this.runToastUtil.success(2000, 'Conta bancária adicionada com sucesso');
        setTimeout(() => {
          this.sendActiveNav.emit('bank');
        }, 2000);
      }, 
      error => {
        this.runToastUtil.success(2000, 'Erro ao adicionar a conta bancária');
  ...
  update() {
    ...
    this.bankAccountService.update(this.bank_account.id, this.formBankAccount.value).subscribe(
      success => {
        this.runToastUtil.success(2000, 'Conta bancária editada com sucesso');
        setTimeout(() => {
          location.reload();
        }, 2000);
      }, 
      error => {
        this.runToastUtil.error(2000, 'Erro ao editar a conta bnacária');
