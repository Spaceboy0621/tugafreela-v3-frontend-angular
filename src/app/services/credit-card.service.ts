...
export class CreditCardService {
  ...
  create(credit_card: CreditCardModel) {
    return this.http.post<CreditCardModel>(`${ENV.API_URL}/credit-cards`, credit_card);
}

  getById(id: number) {
      return this.http.get<CreditCardModel>(`${ENV.API_URL}/credit-cards/${id}`);
  }

  getAll(){
    return this.http.get<CreditCardModel[]>(`${ENV.API_URL}/credit-cards`);
  }

  update(id: number, credit_card: CreditCardModel) {
      return this.http.put(`${ENV.API_URL}/credit-cards/${id}`, credit_card);
  }

  delete(id: number) {
    return this.http.delete(`${ENV.API_URL}/credit-cards/${id}`);
  }

  getByUser(id: number) {
    return this.http.get<CreditCardModel[]>(`${ENV.API_URL}/credit-cards?card_owner=${id}`)
  }

  hasVerifiedPayment(id: number) {

    this.getByUser(id).subscribe(
      success => {
        success = success.filter(item => item.checked);

        if(success.length > 0) {
          return;
        }
        else {
          this.runToastUtil.error(3000, 'Nenhuma método de pagamento verificado encontrado. Por favor, adicione um ou verifique um existente!');
          setTimeout(() => {
      ...
      error => {
        return ['Erro ao obter metodo de pagamento verificado', error]
      }
    );
